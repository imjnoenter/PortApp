...
const SHEET_ID = '10MpOhXmtTlCVBoohxE3WPz_huT3rj3VkZ1GDwVq3xLQ';
const KEY1 = 'd8c4di9r01qidic6asngd8c4di9r01qidic6aso0';
const KEY2 = 'd8c4e1hr01qidic6aug0d8c4e1hr01qidic6augg';
...
function refreshQuotes() {
  ...
  // Dedupe — the same symbol can now appear in multiple portfolios.
  ...
  // Portfolio symbols → Finnhub (split between 2 keys)
  const mid   = Math.ceil(portSymbols.length / 2);
  const half1 = portSymbols.slice(0, mid);
  const half2 = portSymbols.slice(mid);
  const requests = [
    ...half1.map(s => ({ url: `https://finnhub.io/api/v1/quote?symbol=${encodeURIComponent(s)}&token=${KEY1}`, muteHttpExceptions: true })),
    ...half2.map(s => ({ url: `https://finnhub.io/api/v1/quote?symbol=${encodeURIComponent(s)}&token=${KEY2}`, muteHttpExceptions: true })),
  ];
  ...
  try {
    ...
    const idxUrl = `https://query1.finance.yahoo.com/v7/finance/quote?symbols=${syms}&crumb=${encodeURIComponent(yc.crumb)}`;
    ...
    (JSON.parse(idxRes.getContentText())?.quoteResponse?.result || []).forEach(q => {
      if (q.regularMarketPrice > 0)
        rows.push([q.symbol, q.regularMarketPrice, q.regularMarketChange ?? '', q.regularMarketChangePercent ?? '', nowSec]);
    });
...
function isMarketOpenET_() {
  ...
  return day >= 1 && day <= 5 && hhmm >= '0930' && hhmm < '1610';
}
...
function transferSharesAction(ss, p) {
  ...
  if (toRow > 0) {
    ...
    sh.appendRow(newRow);
    // Copy the Current price / Market Cap formulas from the previous row so the new
    // holding gets a live price (otherwise the web app drops it — no price → not "held").
    const newRowIdx  = sh.getLastRow();
    const prevRowIdx = newRowIdx - 1;
    if (prevRowIdx >= 2) {
      ['Current price', 'Market Cap'].forEach(colName => {
...
function fetchYahooMetadata(sym) {
  try {
    ...
    const url  = 'https://query2.finance.yahoo.com/v10/finance/quoteSummary/'
      + encodeURIComponent(ySym) + '?modules=assetProfile,quoteType&crumb=' + encodeURIComponent(crumb);
