...
- **Extended hours (pre/post-market, Mon–Fri):** no sheet data, and the Yahoo path returns nothing in production — the quote is effectively **all CNBC** via the `fetchQuotes` wrapper (see Volume + Extended-Hours Fetching). Auto-refreshes every 5 min via `isExtendedHours()`.
- **Non-market hours:** same as extended hours (CNBC backfill, Yahoo path dead), manual refresh.
...
## Volume + Extended-Hours Fetching (CNBC)
Stock **and** ETF Volume (Positions → Performance tab) **and all pre/post-market prices** are sourced from **CNBC's public quote endpoint**, not Yahoo or Finnhub — direct client fetch, no Apps Script, no CORS proxy, no API key:
- `fetchCnbcQuotes(symbols)` — batches all non-index (`^`-prefixed) symbols into one request: `https://quote.cnbc.com/quote-html-webservice/quote.htm?symbols=<pipe-separated, URL-encoded>&exthrs=1&output=json`. Response key varies (`QuickQuoteResult.QuickQuote` or `FormattedQuoteResult.FormattedQuote`; a single symbol can come back as an object, not an array). Native dotted-ticker support (`BRK.B`) — no `yahooTicker()` conversion needed. All numbers arrive as **strings** (thousands-separated; `"UNCH"` for a flat change), so every field goes through a comma-stripping `num()`. Returns per symbol: `volume` (**`fullVolume`**, falling back to `volume` — `fullVolume` matches Yahoo's `regularMarketVolume` exactly (verified); plain `volume` is regular-session-only and reads lower), `price`/`change`/`changePct` (regular session), `high52`/`low52` (from `FundamentalData.yrhiprice`/`yrloprice`), and `extType`/`extPrice`.
- **`&exthrs=1` is what adds the `ExtendedMktQuote` block** — without it the response has no extended-hours data at all and `last` stays pinned at the regular close after 16:00 ET. `ExtendedMktQuote.type` is `PRE_MKT`/`POST_MKT` (mapped to the app's `marketState` `'PRE'`/`'POST'`) and `.last` is the extended-hours price. This is **the only pre/post-market price source still reachable from a browser** — the old `fetchSingleQuoteFromChart` path that computed `postMarketPrice` from `v8/chart?includePrePost=true` is production-dead (no CORS headers, and the corsproxy retry is localhost-only), which is why after-hours prices silently showed nothing on the deployed site while working in local dev.
- `fetchQuotes(symbols)` is a thin wrapper around the original quote logic (renamed `_fetchQuotesBase`) that merges `fetchCnbcQuotes` results into every returned entry, across all three quote paths (sheet/Yahoo/chart fallback) in one place. Volume always overwrites; **everything else only fills a `null`/absent field**, so during market hours the Finnhub-backed `Quotes` sheet stays authoritative. Outside market hours `_fetchQuotesBase` returns `{}` (both Yahoo paths are dead), so CNBC becomes the entire quote — without the backfill, `attachQuotes` would null out `dayChange`/`week52High`/`week52Low` for every position after the close. `attachQuotes()` itself is unchanged — it already read all these fields.
...
- **Donuts:** `renderGroupedDonut(cfg, model)` backs the sector / industry / category donuts. `renderSectorDonut` / `renderIndustryDonut` / `renderCategoryDonut` are thin wrappers around the `SECTOR_DONUT` / `INDUSTRY_DONUT` / `CATEGORY_DONUT` configs (grouping field, element id, drilldown-state accessors, post-drill side effects). Drilldown/instance state stays in module-level vars (`sectorDrilldown`, `catDonutInst`, …) read by `setSectorFilter` / `setCategoryFilter` / `rerenderDonuts`. To add a donut, add a config — don't fork the renderer. **Exception:** `renderSectorDonut`/`renderIndustryDonut` branch to a wholly separate static-donut path for the ETF portfolio tab — see Multi-Portfolio.
  - **Donut tooltip return line:** `formatReturnTipLine(pnl, pct)` (shared formatter) + `symbolReturnTipLine(sym, model)` (per-symbol lookup, `''` for non-position slices) back a second tooltip line showing total return. `renderGroupedDonut` and `_netWorthDonutData` each build a `rets[]` array parallel to `labels`/`data` — drilled: per-symbol `{pnl, pct}` from `p.pnl`/`p.pnlPct`; un-drilled: per-group `{pnl, pct: pnl/cost*100}` aggregated from `p.pnl`/`p.costBasis` (matches `renderPnLBySector`'s math exactly), or `null` when cost is 0. Tooltips index `rets` by `c.dataIndex`, not by label, since un-drilled labels are translated via `t(k)`. Covers the Total Net Worth donut, the grouped sector/industry/category/custom/my-group donuts (both levels), symbol-detail modal donuts, and plan-panel expand donuts. **Not** the action-modal ALLOCATION-tab planner donuts (target %, has a non-symbol "Available" slice) or the ETF look-through donuts (blended exposure has no cost basis, so `rets` is omitted and the column renders blank).
  - **Percentage denominator contract:** every donut's tooltip %, its outer label %, and (where present) its legend/residual % must divide by the *same* total. `donutOuterLabels` defaults to the sum of the plotted slices; a chart whose slices don't cover its whole total must pass the real denominator as `options.plugins.donutOuterLabels.total` (`_renderEtfStaticDonut` does this from its `totalOverride`), otherwise the outer labels silently over-report. The ETF look-through donuts are the only charts where slices ≠ total — their denominator is the whole ETF-portfolio value, so slice % + the "Other/Unclassified" residual % sum to 100 on both the card and the modal.
  - **Fullscreen donut modal legend:** `_buildDonutLegend(labels, data, colors, total, showResidual, rets)` renders a `.dm-ret` column. Legend rows are value-sorted for display but each retains its original `idx`, so `rets` must be looked up by the pre-sort `r.idx`, not the row's display position. Residual row gets an empty `.dm-ret` cell for column alignment.
...
- **True Exposure (look-through) card** (`renderTrueExposure`, `id="trueExposureCard"`, side panel below Risk & Concentration; **ALL + ETF views only**): `computeTrueExposure(model)` blends each direct (non-ETF) holding's own weight with the weight it carries *inside* every ETF's cached constituents (`_blendedEtfHoldings` for the via-ETF part), so a stock held both directly and via VOO/QQQ shows its real combined weight (`direct` + `viaEtf` = `total`). A row's total is honey-tinted when the ETF portion adds ≥50% on top of the direct weight (hidden concentration). Only ETFs' cached top holdings are decomposed; the uncounted remainder is surfaced as a residual note (`#trueExposureNote`, `other_unclassified_label`). Returns `null` (card hidden) when no ETF look-through data is cached — so it silently no-ops on view-only sheets without Apps Script. Called from the ETF-data-apply block and `renderModelViews`.
...
- `computeForwardIncome(model)` → `{ gross, net, payers, fwdYield, yieldOnCost }` or `null`. Projected annual income = Σ (trailing-12-month per-share dividends from `historyCache.dividends` × **current** shares `p.qty`), withholding-adjusted for net. `fwdYield` = net / current equity value (excl. cash); `yieldOnCost` = net / total `costBasis`. Rendered as a stats strip atop the Dividend Breakdown card (`forwardIncomeStrip()`), so it re-renders wherever `renderDividendBreakdown` does (incl. `toggleCurrency`).
- **Risk-adjusted metrics** (Risk panel §5): `computeReturnSeries(txs)` → `{ rp, rs }` **time-weighted** daily return arrays for portfolio and SPY, stripping each day's external cash flow (`(V_end − netFlow) / V_prev − 1`) so deposits don't fabricate volatility — reuses the holdings-walk from `computeNetWorth`, needs ≥25 history days / ≥20 return points. `computeRiskAdjusted({rp,rs})` → `{ volAnn, maxDD, sharpe, beta, alpha }` (%s except Sharpe/beta): vol = stdev(rp)·√252; maxDD from the cumulative TWR index; Sharpe/alpha use `RISK_FREE_RATE` const (4.5%, top-of-script); beta = cov(rp,rs)/var(rs); alpha = CAPM residual, annualized. `riskAdjusted()` memoizes the pair in `_riskAdjMemo` keyed by `activePortfolio:txCount:spyDayCount` (the panel re-renders every quote tick; a multi-hundred-day series barely moves intraday).
- **Correlation matrix** (`computeCorrelationMatrix(model)` + `renderCorrelationMatrix`, `id="correlationCard"`, **full-width row** on the Allocation page between `.alloc-summary` and `.alloc-donuts`, **all views**): top-10 holdings by `currentValue`; pairwise Pearson correlation of daily returns from `historyCache.prices` over each pair's overlapping dates. Pairs with `<30` common return-days render muted with `—` (never a noisy value); diagonal = 1. Table is `width:100%;table-layout:fixed` (3rem label col + even value cols). `corrColor(r)` is **theme-aware, red↔white↔green** — `color-mix(in oklab, var(--loss|--gain) pct%, var(--card))` with `pct = 8 + |r|·34` (%), `--ink` text: red (`--loss`) as r→+1 (move together = concentration risk), green (`--gain`) as r→−1 (diversifier), `--card` surface as the neutral "white" (stays legible in dark themes). No hard-coded hex. Cells show `r.toFixed(2)` (11px/700). Legend is a theme-token gradient bar (`linear-gradient(90deg, var(--gain), var(--card), var(--loss))`). Returns `null` (card hidden) with <2 qualifying holdings. Called from the ETF-data-apply block, `applyHist` (history load), and `renderModelViews` — **not** any quote-tick path (daily correlation doesn't move intraday).
- **Monthly Returns heatmap** (`computeMonthlyReturns(txs)` + `renderMonthlyReturns(model)`, `id="monthlyReturnsCard"`, **dashboard**, full-width card between `.charts-row` and `.dash-alerts-row`, **all views**): year×month grid of **time-weighted** monthly returns — `computeMonthlyReturns` is a standalone mirror of `computeReturnSeries`'s daily TWR holdings-walk (same guards: SPY history, a first Buy, ≥25 history days / ≥20 return points) but keys each day by `YYYY-MM` and compounds into a monthly return. Returns `{ years:[{year, months[12], yearTotal}], avgMonths[12], avgYear }`, years descending. **Year total = geometric compound** of that year's non-null months; **Avg row = arithmetic mean** per month across years. Only the 12 month cells are color-filled (`color-mix(in oklab, var(--gain|--loss) X%, var(--card))`, `X = min(|r|/0.20,1)·60`, saturating at a **fixed ±20%**, same pattern as `corrColor`); the Year column + Avg row are **signed colored text, no fill**. Month cells format unsigned-positive / 1-decimal / trailing `.0` trimmed (`10%`, `-13.4%`, `-0%`); Year+Avg always signed (`+7.5%`). `title` attr carries full precision. Current (incomplete) month shows plain MTD. Returns `null` (card hidden) on a thin portfolio. Called from the ETF-data-apply block, `applyHist`, and `renderModelViews` — **not** any quote-tick path (monthly TWR only moves on the day's close). i18n: `monthly_returns_title`, `year_label`, `mret_avg_label` (namespaced to avoid the pre-existing `avg_label` avg-cost key).
...
- **Side panel (right):** Plan Watch (alerts only, hidden when none) → Allocation → P&L by Sector → Risk & Concentration → True Exposure (ALL/ETF views only) → **Correlation Matrix (full-width row, all views, hidden with <2 holdings)** → Total Net Worth → Category Breakdown → Sector Breakdown → Industry Breakdown → Calendar → Watchlist → Dividend Breakdown
...
`renderRiskPanel(model)` — pure, idempotent, no state. Five sections:
...
5. **Risk-adjusted (TWR vs SPY)** — annualized volatility, max drawdown, Sharpe, beta & alpha vs SPY. All from `riskAdjusted()` (see Computed Analytics); shows `—` until history has ≥25 trading days.

Called from: sync render block (after `renderSectorBar`), `fetchQuotes().then(...)`, `applyHist` (when history loads), `toggleCurrency`.
...
- Volume + extended-hours quotes (`fetchCnbcQuotes`) and the three Returns-pipeline pings (`backfillMetadata`/`backfillReturns`/`refreshReturns`) all ride this same cadence — they fire every `init()`, not on a separate timer, and rely on their own cooldowns (not this loop) to avoid over-fetching.
